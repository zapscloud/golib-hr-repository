...
func (p *VisaTypeMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrVisaTypes)
...
func (p *VisaTypeMongoDBDao) Get(visa_typeId string) (utils.Map, error) {
	...
	log.Println("accountMongoDao::Get:: Begin ", visa_typeId)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrVisaTypes)
	log.Println("Find:: Got Collection ")

	filter := bson.D{
		{Key: hr_common.FLD_VISA_TYPE_ID, Value: visa_typeId},
		{Key: hr_common.FLD_BUSINESS_ID, Value: p.businessId},
		{Key: db_common.FLD_IS_DELETED, Value: false}}
...
func (p *VisaTypeMongoDBDao) Delete(visa_typeId string) (int64, error) {
	...
	log.Println("accountMongoDao::Delete - Begin ", visa_typeId)
