...
func (p *Staff_categoryMongoDBDao) InitializeDao(client utils.Map, businessId string, staffId string) {
	log.Println("Initialize Staff_category Mongodb DAO")
	p.client = client
...
func (p *Staff_categoryMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrStaff_categorys)
...
func (p *Staff_categoryMongoDBDao) Get(account_id string) (utils.Map, error) {
	...
	log.Println("Staff_categoryMongoDao::Get:: Begin ", account_id)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrStaff_categorys)
	log.Println("Find:: Got Collection ")

	filter := bson.D{
		{Key: hr_common.FLD_STAFF_CATEGORY_ID, Value: account_id},
		{Key: hr_common.FLD_BUSINESS_ID, Value: p.businessId},
		{Key: db_common.FLD_IS_DELETED, Value: false}}
	// Append StaffId in filter if available
	if len(p.staffId) > 0 {
	...
	log.Println("Staff_categoryMongoDao::Get:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *Staff_categoryMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("Staff_categoryMongoDao::Find:: Begin ", filter)
	...
	log.Println("Staff_categoryMongoDao::Find:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *Staff_categoryMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Business Staff_category Save - Begin", indata)
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrStaff_categorys)
	if err != nil {
		return indata, err
	}
	...
	log.Println("Save - End", indata[hr_common.FLD_STAFF_CATEGORY_ID])
...
func (p *Staff_categoryMongoDBDao) UpdateMany(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Staff_categoryMongoDao::UpdateMany - Begin")
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrStaff_categorys)
	if err != nil {
		return utils.Map{}, err
	}
	...
	log.Println("Staff_categoryMongoDao::UpdateMany - End")
	return indata, nil
}
...
func (p *Staff_categoryMongoDBDao) Delete(account_id string) (int64, error) {
	...
	log.Println("Staff_categoryMongoDao::Delete - Begin ", account_id)
	...
	log.Printf("Staff_categoryMongoDao::Delete - End deleted %v documents\n", res.DeletedCount)
...
func (p *Staff_categoryMongoDBDao) DeleteMany() (int64, error) {
	...
	log.Println("Staff_categoryMongoDBDao::DeleteMany - Begin ", p.businessId, p.staffId)
	...
	log.Printf("Staff_categoryMongoDBDao::DeleteMany - End deleted %v documents\n", res.DeletedCount)
	return res.DeletedCount, nil
}

// func (p *Staff_categoryMongoDBDao) appendListLookups(stages []bson.M) []bson.M {
